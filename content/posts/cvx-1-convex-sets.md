...
\\[B({x_c},r) = \\{ {x\mid {{\lVert  {x - {x_c}} \rVert }_2} \le r} \\} = \\{ {x\mid {{(x - {x_c})}^T}(x - {x_c}) \le {r^2}} \\}\\]
...
<details>
    ...
    <p>对于两个球 \({\lVert  {{x_1} - {x_c}} \rVert _2} \le r,{\lVert  {{x_2} - {x_c}} \rVert _2} \le r\) 和 \(0 \le \theta  \le 1\),</p>
    \begin{align}
        {\lVert  {\theta {x_1} + (1 - \theta ){x_2} - {x_c}} \rVert _2}
        & = {\lVert  {\theta ({x_1} - {x_c}) + (1 - \theta )({x_2} - {x_c})} \rVert _2}\\
        & \le \theta {\lVert  {{x_1} - {x_c}} \rVert _2} + (1 - \theta ){\lVert  {{x_2} - {x_c}} \rVert _2}\\
        & \le r
    \end{align}
    <p>第一个 \(\le\) 因为 \({\lVert   \cdot  \rVert _2}\) 是凸函数 (后续文章会提到), 第二个是根据已知球的定义.</p>
</details>
